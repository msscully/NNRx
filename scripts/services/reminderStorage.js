...
app.factory('reminderStorage', ['CordovaService', 'localNotifications', '$q', function (CordovaService, localNotifications, $q) {
          ...
          if (singleQueue.length !== desiredQueueLength) {
            if (singleQueue.length > desiredQueueLength) {
              ...
              for (var j = initQueueLength-1; j >= desiredQueueLength; --j) {
                // Wrapped in function so there's a closure to preserce the
                // value of the variables. Otherwise they would take on
                // whatever value was the last in the loop for all of them. Yay
                // asynchronous!
  ...
  var cancelNotification = function (notificationId, reminderId) {
    return localNotifications.cancel(notificationId).then(function() {
      ...
      var index = indexOfId(notificationId, reminderQueues[reminderId]);
  ...
  var cancelAndReschedule = function(notificationId) {
    ...
    return scheduleNext(reminderId).then( function () {
      return localNotifications.cancel(notificationId).then( function () {
        delete noteId2ReminderId[notificationId];
        var singleQueue = reminderQueues[reminderId];
        var index = indexOfId(notificationId, singleQueue);
        if (index >= 0){
          reminderQueues[reminderId].splice(index, 1);
        }
        putInLocalStorage(QUEUE_STORAGE_ID, reminderQueues);
  ...
  var deleteReminder = function(reminderId, rebalance) {
    ...
    var singleQueue = reminderQueues[reminderId];
    var notificationCount = singleQueue.length;
  ...
  var getTriggeredNotifications = function() {
    ...
    for (var reminderId in reminders) {
      if (reminders.hasOwnProperty( reminderId )) {
        var singleQueue = reminderQueues.hasOwnProperty(reminderId) ? reminderQueues[reminderId] : [];
    ...
    return triggeredNotifications.sort(compare('date'));
  };

  var indexOfId = function(idToFind, array) {
    var len = array.length;
    for (var i = 0; i < len; ++i) {
      if (parseInt(idToFind) === parseInt(array[i].id)) {
        return i;
      }
    }
    return -1;
  ...
  var addSnooze = function (origNotificationId) {
    ...
    var index = indexOfId(origNotificationId, reminderQueues[reminderId]);
  ...
  var triggeredNotifications = getTriggeredNotifications();

  var updateTriggeredNotifications = function() {
    triggeredNotifications = getTriggeredNotifications();
  };

  CordovaService.resumed.then( function() {
    updateTriggeredNotifications();
  });
  ...
  return {
    ...
    updateTriggeredNotifications: updateTriggeredNotifications,
